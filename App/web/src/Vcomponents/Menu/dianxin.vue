<template>
    <div class="colddrink">
        ...
        <ul class="coldUl">
          ...
          <img :src="item.imgurl"><div class="spe coldDiv5"><span>{{item.name}}</span><p class="speP"><span>{{item.price}}</span><el-input-number class="coldInput" v-model="'num'+item.id" :min="0" :max="20" label="0" lazy></el-input-number></p></div>
          </li>
        </ul>
        </div>
        <bottomnav></bottomnav>
...
<script>
    ...
    setTimeout(function(){
      for(let $i=0;$i<$('.coldDiv5').length;$i++){
        $($('.coldDiv5')[$i]).click(function(event){
