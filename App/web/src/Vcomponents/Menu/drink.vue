<template>
    <div class="colddrink">
        ...
        <ul class="coldUl">
          ...
          <img :src="item.imgurl"><div class="spe coldDiv6"><span>{{item.name}}</span><p class="speP"><span>{{item.price}}</span><el-input-number class="coldInput" v-model="'num'+item.id" :min="0" :max="20" label="0" lazy></el-input-number></p></div>
          </li>
        </ul>
        </div>
        <bottomnav></bottomnav>
...
<script>
  ...
  $(document).ready(function(){
    ...
    setTimeout(function(){
      for(let $i=0;$i<$('.coldDiv6').length;$i++){
        $($('.coldDiv6')[$i]).click(function(event){
